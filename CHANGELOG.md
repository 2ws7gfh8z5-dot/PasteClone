...
### 🎉 全新名字：PasteClone → Just Paste

- **应用更名为 Just Paste**：菜单栏、面板标题、偏好设置与系统权限弹窗中的名称全部更新（内部 bundle id 不变，历史收藏数据不丢失）
- **全新手绘风图标**：App 图标、深色模式图标与菜单栏图标统一换为奶油色剪贴板 + 橙色夹子的 Anthropic 手绘风格
- **电脑只保留一个正式版本**：安装到 /Applications/Just Paste.app，旧 PasteClone / JustPaste 副本已移除

### 🎨 主界面可读性与排版修复

- **任何壁纸下都看得清**：面板背景不透明度从 0.72 提升到 0.92，文字与按钮不再被桌面背景干扰
- **类型标签（文本/图片/富文本）改用主文字色**，来源 App 名称对比度提升
- 按钮排版整理：每行「粘贴到当前输入框」按钮与固定/队列/删除按钮全部可点可用

### ⚠️ 跨平台客户端已知调整

- Windows/Linux 客户端（Rust + egui）暂以窗口内 Ctrl+Shift+V 呼出，真正的系统级全局热键因依赖兼容问题暂时移除，下个双周版本恢复
