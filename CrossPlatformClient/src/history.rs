...
impl Clip {
    pub fn new(text: String) -> Self { Self { text, pinned: false } }
    pub fn hash(&self) -> String { format!("{:x}", Sha256::digest(self.text.as_bytes())) }
}
...
pub fn insert(items: &mut Vec<Clip>, text: String, limit: usize) {
    ...
    let retained = items.iter().find(|item| item.hash() == hash && item.pinned).cloned();
