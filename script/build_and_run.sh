...
xcodebuild -project "$ROOT_DIR/PasteClone.xcodeproj" -scheme "$APP_NAME" -configuration Debug \
...
case "$MODE" in
  ...
  --telemetry|telemetry) open_app; /usr/bin/log stream --info --style compact --predicate 'subsystem == "com.you.PasteClone"' ;;
